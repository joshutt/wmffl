...
$season = $currentWeek == 0 && $season == $currentSeason && date('m') != 8 ? $currentSeason - 1 : $season;
//$season=2025;
...
$lastUpdate = getLastUpdate($entityManager);
...
$title = "$season WMFFL Financial Statements";
...
<h5 align="center">Updated <?= $lastUpdate ?></h5>
...
<div class="center">
    ...
    <?php
        $linkString = '/history/'.($season-1).'Season/teammoney';
        if ($season >= 2025) {
            $linkString = '/history/teammoney?season='.($season-1);
        }
    ?>
    <p>Previous column is based on <a
                href="<?= $linkString ?>"><?= $season - 1 ?>
            results</a></p>
