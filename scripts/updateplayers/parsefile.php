...
//                    print "$statid-";
            ...
            break;
        case 4:     // assume for now this is always date,
            readint($handle);  // ignore first int
            $value = readint($handle);
            $value = $value >> 2;   // shift end 0s off
            $value &= 65535;  // Drop the leading 1s
            $date = EncodeDate(1899, 12, 31) + $value;
            $transaction->setDate($date);
            break;
        }
        $fieldid = readint($handle);
